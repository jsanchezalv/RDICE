...
common_all_inputs <-add_items({
                      ...
                      drq <- 0.035
                      }) #different values than what's assumed by default


#Put objects here that do not change as we loop through treatments for a patient
common_pt_inputs <- add_items({
  death <-  max(0.0000001,rnorm(n=1, mean=12, sd=3))
  }) 
...
unique_pt_inputs <- add_items({fl.sick <-  1
                             q_default <- util.sick
...
```{r model_evts}
init_event_list <- 
  add_tte(arm=c("noint","int"), evts = c("sick","sicker","death"), other_inp = "dummy" ,input={
    sick <- 0
    sicker <- draw_tte(1,dist="exp", coef1=coef_noint, beta_tx = ifelse(arm=="int",HR_int,1))
...
df_par <- data.frame(parameter_name = c("util.sick","util.sicker","cost.sick","cost.sicker","cost.int","coef_noint","HR_int"),
                              ...
                              b=c(0.8,0.5,3000,7000,1000,0.1,0.8)/5,
